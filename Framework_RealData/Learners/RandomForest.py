...
class RandomForest():
	...
	def get_h(self, X):
		self.dtree.fit(self.data_instance.X[X], self.data_instance.y[X])
		h = self.dtree.predict(self.data_instance.X)
	...
	def set_full_data_acc(self):
		self.restart()
		self.dtree.fit(self.data_instance.X, self.data_instance.y)
		h = self.dtree.predict(self.data_instance.X)
	...
	def restart(self):
		self.dtree = RandomForestClassifier(random_state=self.seed, n_estimators=10)
